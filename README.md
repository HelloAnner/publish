...
| 平台 | 图片 | 标题 | 正文 | 草稿 | 说明 |
| --- | --- | --- | --- | --- | --- |
| 小红书 `xhs` | 最多 9 张（必须） | ≤ 20 字 | ≤ 1000 字 | ✔ | 无图自动生成「纸感」封面，失败则退化为「文字配图」模式 |
...
小红书必须有图，所以没有配图时会用本地 Pillow 生成一张 3:4 封面（按内容 hash 缓存，同样的内容不会重复渲染）：
...
publish -c "..." -to xhs --cover none              # 完全不用封面，走小红书自带的文字配图
```

封面细节：正文首段的一小段文字会作为底部灰色副标题（自动去掉结尾标点）；中文按**禁则**处理，
`。，、` 这类标点不会被挤到行首。想固定下来就写进配置：

```json
{ "cover": { "style": "纸感", "scale": 1 } }
```

> 注意：封面渲染样式（`cover.style`）和小红书「文字配图」的样式（`cover.cardStyle`，取值是
> `基础/边框/备忘/…`）是两回事——后者只在 `--cover none` 或封面渲染失败时才会用到。
## 节奏与风控设计
