...
| `--xhs-longform <off\|cards\|auto>` | `off` | 小红书超 1000 字时：报错 / 切成最多 9 张文字卡片 / 超过才自动切 |
...
| --- | --- | --- | --- | --- | --- | --- |
| 小红书 | `xhs` `xiaohongshu` `小红书` | 最多 9 张（必须） | ≤ 20 字 | ≤ 1000 字 | ✔ | 无配图时自动生成「纸感」封面；超 1000 字见 [小红书的长内容](#小红书的长内容) |
...
图文笔记的正文上限是 1000 字。超过时默认会报错并列出可选路径：

```bash
publish -f long.md -to xhs --dry-run                     # 报错 + 提示可选方案
publish -f long.md -to xhs --xhs-longform cards          # 全文切成最多 9 张文字卡片
publish -f long.md -to xhs --xhs-longform auto           # 超过 1000 字才自动切
publish -f long.md -to xhs --allow-truncate              # 截断到 1000 字（丢失结尾）
```

`cards` 走的是 opencli 原生的「文字配图」能力：把正文按段落切成最多 9 张卡片作为笔记图片
（卡片之间用 `|||` 分隔，卡内换行转义为字面量 `\n`），笔记正文则截断到 1000 字并给出告警。
此模式下不会额外生成封面图——卡片本身就是图片。
...
1. **小红书「写长文」入口无法自动化（已实测确认）**。opencli 的 `xiaohongshu publish` 只覆盖图文笔记。
   实测环境：同一个已登录的浏览器会话中，图文发布页（`?target=image`）完全正常；但无论是直接访问
   `?target=article`，还是点击创作中心头部标签里的「写长文」，都会被重定向到
   `/login?redirectReason=401`；`browser tab list` 显示始终是同一个标签页，不存在新开标签或跳错标签的问题。
   结论：这是平台对长文入口的额外鉴权或灰度限制，不是 `publish` 的登录态问题。
   超过 1000 字的替代路径见[小红书的长内容](#小红书的长内容)。
